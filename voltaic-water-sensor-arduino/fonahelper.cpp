...
void checkForNetwork(void){
  int count = 0;
  while ((fona.getNetworkStatus() != 1) && (count < 25)) { // Check network status every 5 seconds
    Watchdog.reset();
  ...
  if (count == 25){ // Send FONA to sleep
    Watchdog.reset();
    Serial.println("Sleeping for ~1hr.");
    setFonaPowerDownMode();
    count += 1;
  }
  while((count > 25) && (count < 450)){ // Sleep for 1 hr
    Watchdog.reset();
  ...
  if (count == 450){ // Reset microcontroller setup sequence by triggering watchdog
    delay(10000);
  }
}
